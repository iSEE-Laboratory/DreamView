...
class ImageLogger(Callback):
    ...
    @rank_zero_only
    def log_texts(self, save_dir, split, texts,
                  global_step, current_epoch, batch_idx):
        root = os.path.join(save_dir, "images", split)
        filename = "{}_gs-{:06}_e-{:06}_b-{:06}.txt".format('texts',
                                                            global_step,
                                                            current_epoch,
                                                            batch_idx)
        path = os.path.join(root, filename)
    ...
    @rank_zero_only
    def log_g_texts(self, save_dir, split, g_texts,
                    global_step, current_epoch, batch_idx):
        root = os.path.join(save_dir, "images", split)
        filename = "{}_gs-{:06}_e-{:06}_b-{:06}.txt".format('g_texts',
                                                            global_step,
                                                            current_epoch,
                                                            batch_idx)
        path = os.path.join(root, filename)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if self.log_all_val and split == "val":
            ...
            if should_log and (check_idx not in self.mp or self.mp[check_idx] < 1):
                if check_idx not in self.mp:
                    self.mp[check_idx] = 1
                else:
                    self.mp[check_idx] += 1
            else:
        ...
        if (should_log and (check_idx % self.batch_freq == 0) and
            ...
            texts = copy.deepcopy(images['text'])
            self.log_texts(pl_module.logger.save_dir, split, texts,
                           pl_module.global_step, pl_module.current_epoch, batch_idx)
            del images['text']

            g_texts = copy.deepcopy(images['g_text'])
            self.log_g_texts(pl_module.logger.save_dir, split, g_texts,
                             pl_module.global_step, pl_module.current_epoch, batch_idx)
            del images['g_text']
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not opt.finetune_from == "":
            msg = model.load_state_dict(torch.load(opt.finetune_from, map_location='cpu'), False)
            print('Loading pre-trained', msg)
